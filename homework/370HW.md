...
First identify that without control unit’s latency time, the most critical instruction is the load instruction, which has **Instr. Mem, Mux, Reg File, Mux, ALU, Data Mem, Mux and Reg File ** along the path. 

1. $180$[ps].

   Before `MemWrite` is used, **Data Memory** needs Address and Write data. After **Instr. Mem**, Control get its input and could generate `MemWrite`. The critical path is $200+30+100+30+120+300+30+100=910[ps]$, which decides the clock cycle time. As `MemWrite` is a write enable signal for the **Data Mem**. So the time is $$910-200-300-30-100=280[ps]$$.

2. `ALUOp`, 110[ps].  

   Analyze each control signal generated by **Control Unit**. 

   - `RegWrite` : before one **Reg File**, $910-200-100=610[ps]$ at most.
   -  `Jump` need to finish before one **Mux** before next rising edge, $910-200-30=680[ps]$ at most.
   -  `RegDst` and `MemtoReg` need to finish before one **Mux and Reg File** before next rising edge, $910-200-30-100=580[ps]$ at most.
   -  `Branch` needs to be generated before two **Mux** before next rising edge, $910-200-30-30=650[ps]$ at most. 
   - `MemRead` is needed before **Data Mem**, one **Mux** and **Reg File**, $910-200-300-30-100=180[ps]$ at most.
   -  `ALUOp` is needed before **ALU Ctrl, ALU** , **Data Mem** and one **Mux, RegFile**, $910-200-50-120-300-30-100=110[ps]$. 
- `ALUSrc` is needed before **Mux, ALU, Data Mem, Mux, RegFile**, $910-200-30-120-300-30-100=130[ps]$ at most. 
