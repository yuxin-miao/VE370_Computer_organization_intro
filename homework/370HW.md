...
2. 
   ...
   - **IN ID STAGE:** If we add NOP as the previous question states, if we assume taken at first, for this particular program will achieve more speed up. The execution will be like (*beq* represents not taken, **beq** means taken)
...
1. Because the branch outcome is determined in the `EX` stage, for every time the mis-predicted branch will need flush previous three instructions. $30\% \times 55\%=16.5\%$ will add extra three clock cycles. Then the extra CPI is $ 30\% \times (1-45\% )\times 3=0.495$. 
2. Assume we will not jump. the jump outcome is determined in the `ID` stage, so one extra clock cycle needed. $30\% \times (1-55\% )\times 3+ 5\% \times1=0.455$ 
