...
1. Because the branch outcome is determined in the `EX` stage, for every time the mis-predicted branch will need flush previous three instructions. $30\% \times 55\%=16.5\%$ will add extra three clock cycles. Then the extra CPI is $16.5\% \times 3=0.495$. 
2. Assume we will not jump. the jump outcome is determined in the `ID` stage, so one extra clock cycle needed. $13.5\% \times 3 + 5\% \times1=0.455$ 
